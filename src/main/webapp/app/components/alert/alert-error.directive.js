(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('timebillingApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-timebillingApp-error');
                var entityKey = httpResponse.headers('X-timebillingApp-params');
                if (errorHeader) {
